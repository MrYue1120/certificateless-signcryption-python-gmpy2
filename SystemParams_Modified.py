...
class User(object) :
	...
	def signcrypt( self, Reciever, params, m):
		...
		#	h2 = H2(ID_S, T, m, C) <-- Zq
		h2_value = H2_hash(self.uid, T_value, m, C_value[0], bit_length(params.q))
		#	(x_S + y_S) ** -1 mod q
	...
	def unsigncrypt( self, Sender, params, Signcryption_text):
		...
		#	m' = (T' ** -1) * C mod p
		m_list_un = []
		for x in range( len( C_value ) ) :
			m_list_un.append( t_mod( T_value_un_invert * C_value[x], params.p ) )
		m_un = list2m( m_list_un, bit_length( params.p ) )
		#	h2' = H2(ID_S, T', m', C) <-- Zq
		h2_value_un = H2_hash( Sender.uid, T_value_un, m_un, C_value[0], bit_length(params.q))
...
def m2list( m, bits ) :
	...
	for x in range( m_number ) :
		if x != m_number - 1 :
			temp_str = '1' + m_bin_str[( 2 + x * ( bits - 2 ) ) : ( 2 + (x + 1 ) * ( bits - 2 ) )]
...
def list2m( m_list, bits) :
	m_number = len( m_list )
	m_str = ''
	for x in range( m_number ) :
		m_str = m_str + str( bin( m_list[x] )[3:] )
	m = mpz( m_str, 2 )
	return m
...
if __name__ == "__main__":
	...
	m = mpz(2)**( 1024 - 1 ) + mpz_urandomb( rand, 1024 - 1 )
